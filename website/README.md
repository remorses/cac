...
-   plugins polls for the database `FramerLoginSession` with the key, when it finds it, login is completed and a session is returned
...
-   redirect to `/api/after-framer-login?key=xxx`, here `FramerLoginSession` is saved in the database, user is told to go back to Framer and complete the login
