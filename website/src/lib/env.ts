...
export const env = {
    ...
    GITHUB_COLLABORATORS_EXPORT_CLIENT_ID: process.env.GITHUB_COLLABORATORS_EXPORT_CLIENT_ID,
    GITHUB_COLLABORATORS_EXPORT_CLIENT_SECRET: process.env.GITHUB_COLLABORATORS_EXPORT_CLIENT_SECRET!,
}
...
export function getBuyReactExportPluginUrl({ orgId, email, projectId }) {
    const url = new URL('/react-export-pricing', env.PUBLIC_URL)
    url.searchParams.append('orgId', orgId)
    url.searchParams.append('email', email)
