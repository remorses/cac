...
import { Utils, AsyncDataFetcher } from "./../lib/utils";
...
class UIComponent {
        ...
        this.iframeElement.addEventListener("load", () => {
          ...
          chrome.storage.local.get("vimiumSecret", ({ vimiumSecret }) => {
            ...
            port1.onmessage = (event) => {
              ...
              if (event)
                eventName =
                  (event.data ? event.data.name : undefined) || event.data;
  ...
  handleDarkReaderFilter() {
    ...
    const reverseFilterIfExists = () => {
      ...
      else this.iframeElement.classList.remove(reverseFilterClass);
    };
  ...
  activate(options = null) {
    ...
    this.postMessage(this.options, () => {
      ...
      if (this.options && this.options.focus) this.iframeElement.focus();
      this.showing = true;
    });
  }
