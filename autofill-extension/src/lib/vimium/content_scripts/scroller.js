...
const doesScroll = function (element, direction, amount, factor) {
  ...
  return (
    performScroll(element, direction, delta) &&
    performScroll(element, direction, -delta)
  );
};
...
var firstScrollableElement = function (element = null) {
  ...
  if (!element) {
    ...
    if (
      doesScroll(scrollingElement, "y", 1, 1) ||
      doesScroll(scrollingElement, "y", -1, 1)
    )
  ...
  if (doesScroll(element, "y", 1, 1) || doesScroll(element, "y", -1, 1)) {
    ...
    children.map(
      (child) => (child.area = child.rect.width * child.rect.height)
    );
...
const CoreScroller = {
  ...
  wouldNotInitiateScroll() {
    return (
      this.lastEvent && this.lastEvent.repeat && Settings.get("smoothScroll")
    );
  },
...
const Scroller = {
  ...
  scrollBy(direction, amount, factor, continuous) {
    ...
    if (!getScrollingElement() && amount instanceof Number) {
      if (direction === "x") window.scrollBy(amount, 0);
