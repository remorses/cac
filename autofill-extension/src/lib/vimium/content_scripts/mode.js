...
class Mode {
  ...
  init(options) {
    ...
    if (this.options.suppressAllKeyboardEvents) {
      ...
      this.options["keydown"] = (event) =>
        this.alwaysSuppressPropagation(() => {
          if (downHanlder) return downHanlder(event);
        });
      const pressHandler = this.options["keypress"];
      this.options["keypress"] = (event) =>
        this.alwaysSuppressPropagation(() => {
          if (pressHandler) return pressHandler(event);
        });
    }
    ...
    if (this.options.singleton) {
      ...
      if (singletons[key] != null) singletons[key].exit();
      singletons[key] = this;
    }
  ...
  setIndicator(indicator) {
    if (indicator) this.options.indicator = indicator;
  ...
  exit(...args) {
    ...
    for (let handler of this.exitHandlers) handler(...Array.from(args || []));

    for (let handlerId of this.handlers) handlerStack.remove(handlerId);
  ...
  logModes() {
    if (Mode.debug) {
      ...
      for (let mode of Mode.modes.slice().reverse()) this.log(" ", mode.id);
    }
  }

  log(...args) {
    if (Mode.debug) console.log(...Array.from(args || []));
  }
  ...
  // For tests only.
  static reset() {
    for (let mode of this.modes) mode.exit();
    this.modes = [];
  }
}
...
class CacheAllKeydownEvents extends SuppressAllKeyboardEvents {
  ...
  replayKeydownEvents() {
    return this.keydownEvents.map((event) =>
      handlerStack.bubbleEvent("keydown", event)
    );
  }
}

export { Mode, SuppressAllKeyboardEvents, CacheAllKeydownEvents };
