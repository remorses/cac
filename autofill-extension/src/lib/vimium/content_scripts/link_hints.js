import { forTrusted, chrome } from "./../chrome";
import { Rect } from "./../lib/rect";
import { handlerStack } from "./../lib/handler_stack";
import { Utils } from "./../lib/utils";
import { KeyboardUtils } from "./../lib/keyboard_utils";
import { Settings } from "./../lib/settings";
import { DomUtils } from "./../lib/dom_utils";
import { Mode, CacheAllKeydownEvents } from "./mode";
import { HUD } from "./hud";
...
const HintCoordinator = {
  ...
  sendMessage(messageType, request) {
    ...
    Frame.postMessage(
      "linkHintsMessage",
      Object.assign(request, { messageType })
    );
  },
  ...
  getHintDescriptors({ modeIndex, isVimiumHelpDialog }) {
    ...
    DomUtils.documentReady(() => {
      return Settings.onLoaded(() => {
        ...
        this.sendMessage("postHintDescriptors", {
          hintDescriptors: this.localHintDescriptors
        });
      });
    });
  },
      ...
      return Settings.onLoaded(() => {
        if (
          this.cacheAllKeydownEvents != null
            ? this.cacheAllKeydownEvents.modeIsActive
            : undefined
        ) {
...
class LinkHintsMode {
  ...
  constructor(hintDescriptors, mode) {
    ...
    this.markerMatcher = Settings.get("filterLinkHints")
      ? new FilterHints()
      : new AlphabetHints();
    this.markerMatcher.fillInMarkers(
      this.hintMarkers,
  ...
  onKeyDownInMode(event) {
    ...
    if (
      ...
      if (event.shiftKey) this.tabCount--;
      else this.tabCount++;
      this.updateVisibleMarkers();
    } else if (
      event.key === " " &&
  ...
  updateKeyState({ hintKeystrokeQueue, linkTextKeystrokeQueue, tabCount }) {
    ...
    const {
      linksMatched,
      userMightOverType
    } = this.markerMatcher.getMatchingHints(
      this.hintMarkers,
      tabCount,
      this.getNextZIndex.bind(this)
    );
    if (linksMatched.length === 0) {
      ...
      for (let marker of this.hintMarkers) this.hideMarker(marker);
  ...
  rotateHints() {
    ...
    let marker, stack;
    const localHintMarkers = this.hintMarkers.filter(
      (marker) => marker.isLocalMarker && marker.style.display !== "none"
    );
  ...
  showMarker(linkMarker, matchingCharCount) {
    ...
    for (let j = 0, end = linkMarker.childNodes.length; j < end; j++) {
      ...
      else linkMarker.childNodes[j].classList.remove("matchingCharacter");
    }
  }

  hideMarker(linkMarker) {
    if (linkMarker.isLocalMarker) linkMarker.style.display = "none";
  }
...
class AlphabetHints {
  ...
  hintStrings(linkCount) {
    ...
    while (hints.length - offset < linkCount || hints.length === 1) {
      ...
      for (let ch of this.linkHintCharacters) hints.push(ch + hint);
    }
...
class FilterHints {
  ...
  renderMarker(marker) {
    ...
    if (linkText.length > 35) linkText = linkText.slice(0, 33) + "...";
    marker.innerHTML = spanWrap(
      marker.hintString + (marker.showLinkText ? ": " + linkText : "")
    );
  }

  fillInMarkers(hintMarkers, getNextZIndex) {
    for (let marker of hintMarkers)
      if (marker.isLocalMarker) this.renderMarker(marker);
...
var LocalHints = {
  ...
  getVisibleClickable(element) {
    ...
    const tagName =
      (element.tagName.toLowerCase ? element.tagName.toLowerCase() : null) ||
      "";
  ...
  getLocalHints(requireHref) {
    ...
    if (Settings.get("filterLinkHints")) {
      for (hint of localHints) Object.assign(hint, this.generateLinkText(hint));
    }
    return localHints;
  },
