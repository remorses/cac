...
const initializePreDomReady = function () {
  ...
  const requestHandlers = {
    focusFrame(request) {
      if (frameId === request.frameId) return focusThisFrame(request);
    },
...
const installListener = (element, event, callback) =>
  element.addEventListener(
    ...
    forTrusted(function () {
      ...
      if (isEnabledForUrl) return callback.apply(this, arguments);
      else return true;
    }),
    true
  );
...
var installListeners = Utils.makeIdempotent(function () {
  ...
  return installListener(document, "DOMActivate", (event) =>
...
var Frame = {
    ...
    if (windowIsFocused() || !DomUtils.windowIsTooSmall()) {
      ...
      window.addEventListener(
        ...
        (focusHandler = forTrusted(function (event) {
          if (event.target === window) postRegisterFrame();
        })),
        true
      );
      return window.addEventListener(
        ...
        (resizeHandler = forTrusted(function (event) {
          if (!DomUtils.windowIsTooSmall()) postRegisterFrame();
        })),
        true
      );
    }
  ...
  init() {
    ...
    //   (disconnect = Utils.makeIdempotent(() => this.disconnect()))
    // );
    return window.addEventListener("unload", () => {}, true);
  },
  ...
  return function () {
    if (highlightedFrameElement == null) {
      ...
      highlightedFrameElement = (function () {
        ...
        _styleSheet.innerHTML = `@import url(\"${chrome.runtime.getURL(
...
var focusThisFrame = function (request) {
  ...
  Utils.nextTick(function () {
    ...
    if (request.highlight) flashFrame();
  });
};
...
var checkEnabledAfterURLChange = forTrusted(function () {
  ...
  if (windowIsFocused()) checkIfEnabledForUrl();
});
...
if (root.HelpDialog == null) {
  root.HelpDialog = {
    ...
    toggle(request) {
      ...
      if (this.helpUI != null && this.isShowing()) return this.helpUI.hide();
