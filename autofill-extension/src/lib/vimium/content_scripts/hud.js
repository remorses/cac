...
const HUD = {
  ...
  findMode: null,
  abandon() {
    if (this.hudUI) this.hudUI.hide(false);
  },
  ...
  init(focusable) {
    ...
    if (this.hudUI == null) {
      this.hudUI = new UIComponent(
        ...
        "vimiumHUDFrame",
        ({ data }) => {
          if (this[data.name]) return this[data.name](data);
        }
      );
    }
    // this[data.name]? data
  ...
  search(data) {
    ...
    const matchCount =
      FindMode.query.parsedQuery.length > 0 ? FindMode.query.matchCount : 0;
  ...
  hide(immediate, updateIndicator) {
    ...
    if (this.hudUI != null && this.tween != null) {
      ...
      this.tween.stop();
      if (immediate) {
        if (updateIndicator) Mode.setIndicator();
        else this.hudUI.hide();
      } else {
  ...
  hideFindMode({ exitEventIsEnter, exitEventIsEscape }) {
    ...
    if (exitEventIsEnter) {
      ...
      if (FindMode.query.hasResults) postExit = () => new PostFindMode();
