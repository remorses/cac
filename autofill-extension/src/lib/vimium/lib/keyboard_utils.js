...
const KeyboardUtils = {
  ...
  getKeyChar(event) {
    ...
    if (!Settings.get("ignoreKeyboardLayout")) {
      ...
      if (this.enUsTranslations[key]) {
        key = event.shiftKey
          ? this.enUsTranslations[key][1]
          : this.enUsTranslations[key][0];
  ...
  getKeyCharString(event) {
    ...
    keyChar =
      mapKeyRegistry[keyChar] != null ? mapKeyRegistry[keyChar] : keyChar;
    return keyChar;
  },

  isEscape: (function () {
    let useVimLikeEscape = true;
    Utils.monitorChromeStorage(
      "useVimLikeEscape",
      (value) => (useVimLikeEscape = value)
    );
  ...
  isModifier(event) {
    return ["Control", "Shift", "Alt", "OS", "AltGraph", "Meta"].includes(
      event.key
    );
  },
