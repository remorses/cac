...
if (typeof global == "undefined") globalThis.global = window;
...
var Utils = {
  isFirefox: (function () {
    ...
    if (browserInfo) {
      browserInfo.then(
        (browserInfo) => (isFirefox = browserInfo.name === "Firefox")
      );
    }
    return () => isFirefox;
  })(),
  ...
  firefoxVersion: (function () {
    ...
    if (browserInfo) {
      browserInfo.then(
        (browserInfo) =>
          (ffVersion = browserInfo != null ? browserInfo.version : undefined)
      );
    }
    return () => ffVersion;
  })(),
  ...
  isExtensionPage(win) {
    ...
    try {
      return (
        (win.document.location != null
          ? win.document.location.origin
          : undefined) +
          "/" ===
  ...
  createFullUrl(partialUrl) {
    if (this.hasFullUrlPrefix(partialUrl)) return partialUrl;
    else return "http://" + partialUrl;
  },
  ...
  isUrl(str) {
    ...
    if (dottedParts.length > 1) {
      ...
      if (
        (2 <= lastPart.length && lastPart.length <= 3) ||
        longTlds.includes(lastPart)
      ) {
        return true;
      }
    }
  ...
  convertToUrl(string) {
    ...
    if (Utils.hasChromePrefix(string)) {
      ...
      if (Utils.haveChromeVersion("46.0.2467.2")) return string;
      else return Utils.decodeURIByParts(string);
  ...
  compareVersions(versionA, versionB) {
    ...
    for (
      let i = 0, end = Math.max(versionA.length, versionB.length);
      i < end;
      i++
    ) {
  ...
  makeIdempotent(func) {
    return function (...args) {
      ...
      const result = (([previousFunc, func] = Array.from((ref = [func, null]))),
      ref)[0];
      if (result) {
...
export { Utils, SimpleCache, AsyncDataFetcher, JobRunner };
