...
class HandlerStack {
    ...
    for (let handler of this.stack.slice().reverse()) {
      ...
      if (!(handler != null ? handler.id : undefined) || !handler[type]) {
        ...
        if (result === this.passEventToPage) {
          ...
          result === this.continueBubbling ||
          (result && result !== this.suppressEvent)
        ) {
  ...
  isChromeEvent(event) {
    ...
    return (
      (event != null ? event.preventDefault : undefined) != null ||
...
export { handlerStack, HandlerStack };
