...
var DomUtils = {
  ...
  isSelected(element) {
    ...
    if (element.isContentEditable) {
      ...
      if (
        DomUtils.getSelectionType(selection) === "Range" &&
      ...
      if (element.tagName.toLowerCase() !== "textarea") {
        ...
        try {
          if (element.selectionStart === 0 && element.selectionEnd === 0) {
            return element.setSelectionRange(
  ...
  simulateClickDefaultAction(element, modifiers) {
    ...
    if (
      (element.tagName != null ? element.tagName.toLowerCase() : undefined) !==
        "a" ||
      !element.href
    ) {
      return;
