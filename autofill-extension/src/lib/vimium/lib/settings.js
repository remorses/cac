...
const Settings = {
  ...
  runOnLoadedCallbacks() {
    this.log(
      `runOnLoadedCallbacks: ${this.onLoadedCallbacks.length} callback(s)`
    );
    this.isLoaded = true;
  ...
  shouldSyncKey(key) {
    return (
      ...
      !["settingsVersion", "previousVersion"].includes(key)
    );
  },
  ...
  propagateChangesFromChromeStorage(changes) {
    for (let key of Object.keys(changes || {})) {
      ...
      this.handleUpdateFromChromeStorage(
        key,
        change != null ? change.newValue : undefined
      );
    }
  },
  ...
  get(key) {
    if (!this.isLoaded)
      console.log(
        `WARNING: Settings have not loaded yet; using the default value for ${key}.`
      );
    if (key in this.cache && this.cache[key] != null)
      return JSON.parse(this.cache[key]);
    else return this.defaults[key];
  },
  ...
  set(key, value, shouldSetInSyncedStorage) {
    ...
    this.log(
      `set: ${key} (length=${this.cache[key].length}, shouldSetInSyncedStorage=${shouldSetInSyncedStorage})`
    );
    if (this.shouldSyncKey(key)) {
  ...
  performPostUpdateHook(key, value) {
    if (this.postUpdateHooks[key]) this.postUpdateHooks[key](value);
  },
