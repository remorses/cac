...
describe('typedControls shape', () => {
    let framerClient: Awaited<ReturnType<typeof connect>>

    beforeAll(async () => {
        const projectUrl =
            process.env.FRAMER_PROJECT_URL || defaultServerApiProjectUrl
    ...
    it('inspects typedControls on a component instance', async () => {
        const componentInstances =
            await framerClient.getNodesWithType('ComponentInstanceNode')
        ...
        const summary = Object.fromEntries(
            Object.entries(typed).map(([hashedId, control]) => {
                return [
                    hashedId,
                    {
                        key: control.key,
                        title: (control as { title?: string }).title,
                        type: control.type,
...
describe('serializeAttributesForXml', () => {
    it('serializes page+section link control objects', () => {
        ...
        expect(serializedAttributes.ctaLink).toBe(
            JSON.stringify(linkControlValue),
        )
    })
    ...
    it('serializes image asset-like objects as URL strings', () => {
        ...
        const serializedAttributes = serializeAttributesForXml({
            heroImage: new MockImageAsset(
                'https://framerusercontent.com/images/example.jpg',
            ),
        })

        expect(serializedAttributes.heroImage).toBe(
            'https://framerusercontent.com/images/example.jpg',
        )
    })
    ...
    it('skips remote component schema imports in server runtime', async () => {
        const propertyControls = await getComponentPropertyControls(
            'https://framer.com/m/component-example.js',
        )
